...
#
from wangban_utils.byte_dict_covertion import byte_to_dict,dict_to_byte
...
class DataCleanConnection:
    ...
    def get_one_from_mongo(self,sheet_name,condition=None,field=None):
        data = self.mongo_conn.get_from_db(sheet_name,condition=condition,return_field=field)
        return data

    def get_all_from_mongo(self,sheet_name,condition=None,field=None):
        for single_data in self.mongo_conn.get_all_from_db(sheet_name,condition=condition,return_field=field):
            yield single_data

    def update_from_mongo(self,sheet_name,query,new_info):
        try:
            self.mongo_conn.update_data(sheet_name,query_condition=query,new_data=new_info)
        except Exception as e:
            print('DataCleanConnection update_from_mongo error',e)

    def put_to_mongo(self,data,sheet_name):
        self.mongo_conn.insert_into_db(data,sheet_name)
