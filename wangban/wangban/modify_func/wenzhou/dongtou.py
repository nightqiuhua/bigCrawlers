...
def dongtou_modifier(response,url):
    body = lxml.html.fromstring(response)
    body.make_links_absolute('http://www.dtggzy.com/')
    ...
    if check_table is not None:
        ...
        attachment_url = 'http://www.dtggzy.com/dtcms/attachment_url.jspx?cid={}&n={}'.format(cid,n)
        try:
        ...
        for td_ele,href in zip(check_table.xpath('.//tr//td//a[@title="文件下载"]'),eval(attachment_urls.text)):
            final_href = "http://www.dtggzy.com/dtcms/attachment.jspx?cid={}&i={}{}".format(cid,i,href)
