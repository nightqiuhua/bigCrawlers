...
def wencheng_modifier(response,url):
    body = lxml.html.fromstring(response)
    body.make_links_absolute('http://www.wczbtb.com/')
    ...
    if check_table is not None:
        ...
        attachment_url = 'http://www.wczbtb.com/wzcms/attachment_url.jspx?cid={}&n={}'.format(cid,n)
        try:
        ...
        for td_ele,href in zip(check_table.xpath('.//tr//td//a[@title="文件下载"]'),eval(attachment_urls.text)):
            final_href = "http://www.wczbtb.com/wzcms/attachment.jspx?cid={}&i={}{}".format(cid,i,href)
