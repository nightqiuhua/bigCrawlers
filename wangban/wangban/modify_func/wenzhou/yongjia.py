...
def yongjia_modifier(response,url):
    body = lxml.html.fromstring(response)
    body.make_links_absolute('http://61.164.96.196:7088/')
    ...
    if check_table is not None:
        ...
        attachment_url = 'http://61.164.96.196:7088/yjcms/attachment_url.jspx?cid={}&n={}'.format(cid,n)
        try:
        ...
        for td_ele,href in zip(check_table.xpath('.//tr//td//a[@title="文件下载"]'),eval(attachment_urls.text)):
            final_href = "http://61.164.96.196:7088/yjcms/attachment.jspx?cid={}&i={}{}".format(cid,i,href)
