...
def deqing_modifier(response,url):
    body = lxml.html.fromstring(response)
    body.make_links_absolute('http://www.dqztb.gov.cn/')
    etree.strip_elements(body,"script","style","title",'iframe')
    del_ele = body.xpath('//div[@class="title"]')
    for ele in del_ele:
        ele.clear()
    try:
    ...
    if check_table is not None:
        ...
        attachment_url = 'http://www.dqztb.gov.cn/attachment_url.jspx?cid={}&n={}'.format(cid,n)
        try:
        ...
        for td_ele,href in zip(check_table.xpath('.//tr//td//a[@title="文件下载"]'),eval(attachment_urls.text)):
            final_href = "http://www.dqztb.gov.cn/attachment.jspx?cid={}&i={}{}".format(cid,i,href)
