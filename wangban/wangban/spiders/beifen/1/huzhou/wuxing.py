...
class WuXingSpider(RedisStaticSpider):
    name = 'wuxing'
    ...
    def next_requests(self):
        ...
        while found < self.redis_batch_size:
            ...
            if links_dict['type'] == 'sub':
                yield Request(links_dict['an_sub_url'],meta=links_dict,callback=self.parse_sub_pageurls,errback=self.errorsave,dont_filter=True)
            if links_dict['type'] == 'column':
                yield Request(links_dict['an_column_url'],meta=links_dict,callback=self.parse_urls_incolumn,errback=self.errorsave,dont_filter=True)
