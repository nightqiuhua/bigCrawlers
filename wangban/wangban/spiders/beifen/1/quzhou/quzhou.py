...
JSONFILE = os.path.join(SETTINGS['BASE_JSONFILE_PATH'],'quzhou.json')


class QuZhouSpider(RedisStaticSpider):
    name = 'quzhou'
    start_urls = ['http://www.qzggzy.com/']
    source_website = '衢州市公共资源交易网'
    specific_area = '衢州'
    source_url = 'http://www.qzggzy.com/'
    links_tree = {}
    loss_urls = {}
    column_urls_pool = []
    ...
    def get_totalpage(self,response):
        try:
            total_page = re.findall(r"'1/(\d+)'",response.body.decode('utf-8'))[0]
    ...
    def get_an_title(self,element,response=None):
        an_title = 'NONE'
        try:
            an_title = element.xpath(self.xp.an_title).extract()
    ...
    def cre_page_url(self,f_p_url,page):
        if page == 1:
            page = 'trade'
        page_url = f_p_url.replace('trade.html',self.post_suf.format(page))
        return page_url