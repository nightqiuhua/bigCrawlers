...
@singleton
class HangZhouSpider(DIYBaseSpider):
    name = 'hangzhou'
    ...
    def __init__(self,jsonfile = JSONFILE):
        ...
        self.pre_suf = None
        self.post_url ='https://www.hzctc.cn/SecondPage/GetNotice'
    ...
    def generate_request(self,links_dict=None,spider=None):
        ...
        if links_dict['type'] == 'sub':
            ...
            return FormRequest(self.post_url,meta=links_dict,formdata= formdata,callback=self.parse_sub_pageurls,errback=self.errorsave,dont_filter=True)
        if links_dict['type'] == 'column':
            formdata = links_dict['formdata']
            return FormRequest(self.post_url,meta=links_dict,formdata=formdata,callback=self.parse_urls_incolumn,errback=self.errorsave,dont_filter=True)
...
@singleton
class HangZhouSeleSpider(BaseSeleSpider):
    name = 'hangzhou'
    def __init__(self):
        super().__init__()
