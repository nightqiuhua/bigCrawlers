...
@singleton
class JinHhua_AjaxSpider(DIYBaseSpider):
    name = 'jinhua_ajax'
    ...
    def generate_request(self,links_dict=None,spider=None):
        if links_dict['type'] == 'sub':
            ...
            return FormRequest(links_dict['an_sub_url'],meta=links_dict,formdata= formdata,callback=self.parse_sub_pageurls,errback=self.errorsave,dont_filter=True)
        if links_dict['type'] == 'column':
            formdata = links_dict['formdata']
            return FormRequest(links_dict['an_column_url'],meta=links_dict,formdata=formdata,callback=self.parse_urls_incolumn,errback=self.errorsave,dont_filter=True)
        if links_dict['type'] == 'content':
            link_key = 'http://www.jhztb.gov.cn/platform/noticeController.do?getNotice'
            formdata = links_dict['formdata']
            return FormRequest(link_key,meta=links_dict,formdata=formdata,callback=spider.parse,errback=self.errorsave,dont_filter=True)
